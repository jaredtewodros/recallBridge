// Path: /twilio-click
// Runtime: Node 18
// Purpose: Validate Twilio signature, then forward click webhook to GAS /exec via proxy header.
...
exports.handler = async function (context, event, callback) {
  ...
  while (attempt < 3 && !success) {
    try {
      ...
      console.log("forward click", { code: res.status, body });
      success = res.status >= 200 && res.status < 300;
      if (!success) await sleep(150 * (attempt + 1));
    } catch (err) {
      console.error("forward click error", err);
